...
public class Seeker : MonoBehaviour
{
    public int[] second;
       ...
       void Start()
       {
           ...
           lvlSeeker = PlayerPrefs.GetInt(save);
       ...
       public void PanelsX()
       {
        ...
        panelsX[lvlSeeker - 1].SetActive(true);
       }

       public void UpdateClick()
       {
           if(my_x[0] >= x1[lvlSeeker - 1])
           {
            my_x[0] -= x1[lvlSeeker - 1];
            PlayerPrefs.SetInt("x1", my_x[0]);
            Debug.Log("x1 before" + my_x[0]);
             if(my_x[1] >= x2[lvlSeeker - 1])
            {
              my_x[1] -= x2[lvlSeeker - 1];
              PlayerPrefs.SetInt("x2", my_x[1]);
              Debug.Log("x2 before" + my_x[1]);
               if(my_x[2] >= x3[lvlSeeker - 1])
              {
                my_x[2] -= x3[lvlSeeker - 1];
                PlayerPrefs.SetInt("x3", my_x[2]);
                Debug.Log("x3 before" + my_x[2]);
                 if(my_x[3] >= x4[lvlSeeker - 1])
                {
                  my_x[3] -= x4[lvlSeeker - 1];
                  PlayerPrefs.SetInt("x4", my_x[3]);
                  Debug.Log("x4 before" + my_x[3]);
                   if(my_x[4] >= x5[lvlSeeker - 1])
                  {
                    my_x[4] -= x5[lvlSeeker - 1];
           ...
           PlayerPrefs.SetInt(save, lvlSeeker);
           panelsX[lvlSeeker - 1].SetActive(true);
           panelsX[lvlSeeker - 2].SetActive(false);
           if(lvlSeeker >= 5)
           {
             updatepanelsX5.SetActive(false);
           }
